...
describe("<TreeNode />", () =>
{
    describe("component", () =>
    {
        it("should render the rt-node container", () =>
        {
            const wrapper = shallow(
                <TreeNode {...baseProps} />,
            );

            assert.isTrue(wrapper.find(".rt-node").exists());
        });
    ...
    describe("checked", () =>
    {
        ...
        {
            const iconMap = {
                0: <span className="rt-icon rt-icon-uncheck" />,
                1: <span className="rt-icon rt-icon-check" />,
                2: <span className="rt-icon rt-icon-half-check" />,
            };
    ...
    describe("expandDisabled", () =>
    {
        it("should disable the expand <button>", () =>
        {
            ...
            assert.isTrue(wrapper.find("Button.rt-collapse-btn[disabled]").exists());
        });
    });
    ...
    describe("expanded", () =>
    {
        ...
        it("should render expanded icons when set to true", () =>
        {
            ...
            assert.isTrue(wrapper.contains(<span className="rt-icon rt-icon-expand-open" />));
            assert.isTrue(wrapper.contains(<span className="rt-icon rt-icon-parent-open" />));
        });
        ...
        it("should render collapsed icons when set to false", () =>
        {
            ...
            assert.isTrue(wrapper.contains(<span className="rt-icon rt-icon-expand-close" />));
            assert.isTrue(wrapper.contains(<span className="rt-icon rt-icon-parent-close" />));
        });

        it("should append the `rt-node-expanded` class to the node when set to true", () =>
        {
            const wrapper = shallow(
                <TreeNode {...baseProps} expanded isLeaf={false} />,
            );

            assert.isTrue(wrapper.find("li").hasClass("rt-node-expanded"));
        });

        it("should append the `rt-node-collapsed` class to the node when set to true", () =>
        {
            const wrapper = shallow(
                <TreeNode {...baseProps} expanded={false} isLeaf={false} />,
            );

            assert.isTrue(wrapper.find("li").hasClass("rt-node-collapsed"));
        });
        ...
        it("should not append any expanded/collapsed classes to the node when a leaf", () =>
        {
            ...
            assert.isFalse(wrapper.find("li").hasClass("rt-node-expanded"));
            assert.isFalse(wrapper.find("li").hasClass("rt-node-collapsed"));
        });
    });
    ...
    describe("showCheckbox", () =>
    {
        it("should render a checkbox for the node when true", () =>
        {
            ...
            assert.isTrue(wrapper.contains(
                <span className="rt-checkbox">
                    <span className="rt-icon rt-icon-uncheck" />
                </span>,
            ));
        });
        ...
        it("should not render a checkbox or label for the node when false", () =>
        {
            ...
            assert.isFalse(wrapper.find(".rt-checkbox").exists());
            assert.isTrue(wrapper.find(".rt-bare-label").exists());
        });
    });
    ...
    describe("showNodeIcon", () =>
    {
        it("should render the node icon when true", () =>
        {
            ...
            assert.isTrue(wrapper.contains(
                <span className="rt-node-icon">
                    <span className="rt-icon rt-icon-leaf" />
                </span>,
            ));
        });
    ...
    describe("title", () =>
    {
        ...
        it("should add the `title` property to the bare label when set on a checkbox-less node", () =>
        {
            ...
            assert.equal("Some extra text", wrapper.find(".rt-bare-label").prop("title"));
        });
    });
        ...
        it("should negate the expanded property and pass the current node\"s value", () =>
        {
            ...
            const wrapper = shallow(
                <TreeNode
                    ...
                    isLeaf={false}
                    value="jupiter"
                    onExpand={(node) => { actual = node; }}
                />,
            );

            wrapper.find(".rt-collapse").simulate("click");
    ...
    describe("onClick", () =>
    {
        it("should render the label inside of the DOM label when null", () =>
        {
            ...
            assert.isTrue(wrapper.find("label .rt-title").exists());
        });
        ...
        it("should render the label outside of the DOM label when NOT null", () =>
        {
            ...
            assert.isFalse(wrapper.find("label .rt-title").exists());
        });
